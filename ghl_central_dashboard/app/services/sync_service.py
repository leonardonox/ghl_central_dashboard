...
class GHLSyncService:
    ...
    async def sync_all(self, days_back: int = 7) -> dict:
        ...
        for account in self.accounts.list_active():
            ...
            result['leads_inserted_or_updated'] += self._upsert_leads(account.id, leads)

            opportunities = await self._fetch_opportunities(client, account.location_id, start_date)
            result['opportunities_inserted_or_updated'] += self._upsert_opportunities(account.id, opportunities)

            conversations = await self._fetch_conversations(client, account.location_id)
            result['conversations_inserted_or_updated'] += self._upsert_conversations(account.id, conversations)
            result['accounts'] += 1
        return result
